...
function loadBlankPageDocuments(): BlankPageDocument[] {
  ...
  if (!fsSync.existsSync(filePath)) {
    console.error('❌ blank-page-documents.json not found!');
...
async function reconvertDocument(doc: BlankPageDocument): Promise<ReconversionResult> {
  ...
  try {
    ...
    try {
      ...
      console.log('   🗑️  Deleting existing page images...');
      ...
      console.log(`   📊 New average size: ${newAverageSizeKB.toFixed(2)} KB`);
      console.log(`   ⏱️  Conversion time: ${(conversionTimeMs / 1000).toFixed(2)}s`);

      // Check if reconversion was successful
      if (newAverageSizeKB < 10) {
        console.log(`   ⚠️  WARNING: New average size still < 10 KB!`);
      } else {
        console.log(`   🎉 Success! Pages are no longer blank`);
      }
...
async function processBatch(
  ...
  for (let i = 0; i < documents.length; i += batchSize) {
    ...
    console.log('\n' + '='.repeat(80));
    console.log(`\n📦 Batch ${batchNumber}/${totalBatches} (${batch.length} documents)\n`);

    if (dryRun) {
      console.log('🔍 DRY RUN - Would reconvert:');
...
async function reconvertBlankPageDocuments(): Promise<void> {
  ...
  console.log('🔄 Reconverting Documents with Blank Pages');
  ...
  if (specificDocumentId) {
    ...
    if (!specificDoc) {
      console.error(`\n❌ Document ${specificDocumentId} not found in blank-page-documents.json`);
      console.log('   Run: npm run identify-blank-pages first');
      process.exit(1);
    }
  ...
  if (stillBlank.length > 0) {
    console.log('\n' + '='.repeat(80));
    console.log('\n⚠️  STILL BLANK AFTER RECONVERSION\n');
  ...
  console.log(`\n📄 Exported results to: ${exportPath}`);
...
if (require.main === module) {
  reconvertBlankPageDocuments()
    .then(() => {
      console.log('\n✅ Reconversion complete!');
      process.exit(0);
    })
    .catch((error) => {
      console.error('\n💥 Reconversion failed:', error);
      process.exit(1);
    });
}
